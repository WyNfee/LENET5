%This Script is targeting on using leaky-relu neuron and BGD to do a digit recognition
...
%the architecture:
%input: 400 neurons
%hidden layer: 100 neurons, full connection, activation: leaky-relu
...
if(g_do_gradient_descent == true)
    ...
    for i = 1: iteration_time
        ...
        [t_cost_param, t_gradient_param] = function_Ref_CostFunctionLeaky_Softmax_BGD(t_packedweightforDescent,g_input_data, g_input_answer, g_layer_one_size, g_layer_two_size, g_reularization_param);
    ...
    if(s == 'y')
        save('data_gradient_descent_leaky_softmax.mat', 't_packedweightforDescent', 't_record_cost_data');
    else
    ...
    load('data_gradient_descent_leaky_softmax.mat');
    
    t_cost_data_size = length(t_record_cost_data);
    t_cost_data_leaky = zeros(t_cost_data_size, 2);
    
    for i = 1 : t_cost_data_size
        
        t_cost_data_leaky(i, 1) = i;
        t_cost_data_leaky(i, 2) = t_record_cost_data(i);
        
    end
    
    plot(t_cost_data_leaky(:,1), t_cost_data_leaky(:,2),'--');
    
    s = input('save the loss data?, y to save:','s');
    if(s == 'y')
        save('data_loss_leaky_softmax.mat', 't_cost_data_leaky');
    end
end
...
while(t_test)
    ...
    t_layer_one_data = function_Utils_LeakyFunction(t_picked_Image_data * t_layer_one_weight');
...
t_layer_one_data = function_Utils_LeakyFunction(t_input_data_for_evaluate * t_layer_one_weight');
