...
function [r_cost, r_gradient] = function_Ref_CostFunctionReLu_Softmax_BGD(p_input_weight, p_input_data, p_answer_data, p_layer_one_weight_size, p_layer_two_weight_size, p_regularization_param)
    %unpack the weight
    ...
    %the layer two input
    %add additonal 1 column at the begining
    t_layer_two_input_data = function_Utils_ReLuFunction(t_layer_one_output_data);
   ...
   t_layer_two_error = t_layer_three_error * t_layer_two_weight;
    ...
    t_layer_one_error = t_layer_two_error .* function_Utils_ReLuGradientFunction(t_layer_one_output_data);
