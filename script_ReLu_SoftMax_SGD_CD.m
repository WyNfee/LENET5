...
if(g_do_gradient_descent == true)
    ...
    for i = 1: t_iteration_time
        ...
        [t_cost_param, t_gradient_param] = function_Ref_CostFunctionReLu_Softmax_BGD(t_packedweightforDescent,t_rand_picked_data, t_rand_picked_answer, g_layer_one_size, g_layer_two_size, g_reularization_param);
    ...
    if(s == 'y')
        save('data_sgd_relu_softmax.mat', 't_packedweightforDescent', 't_record_cost_data');
    else
    ...
    load('data_sgd_relu_softmax.mat');
    ...
    s = input('save the loss data?, y to save:','s');
    if(s == 'y')
        save('data_loss_relu_softmax_sgd.mat', 't_cost_data_relu');
    end
end
