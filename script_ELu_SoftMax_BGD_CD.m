%This Script is targeting on using ELu neuron and BGD to do a digit recognition
...
%the architecture:
%input: 400 neurons
%hidden layer: 100 neurons, full connection, activation: Elu
...
%assign the elu hyper parameter
t_elu_param = 1;
...
if(g_do_gradient_descent == true)
    ...
    for i = 1: iteration_time
        ...
        [t_cost_param, t_gradient_param] = function_Ref_CostFunctionELu_Softmax_BGD(t_packedweightforDescent,g_input_data, g_input_answer, g_layer_one_size, g_layer_two_size, g_reularization_param, t_elu_param);
    ...
    if(s == 'y')
        save('data_gradient_descent_elu_softmax.mat', 't_packedweightforDescent', 't_record_cost_data');
    else
    ...
    load('data_gradient_descent_elu_softmax.mat');
    
    t_cost_data_size = length(t_record_cost_data);
    t_cost_data_elu = zeros(t_cost_data_size, 2);
    
    for i = 1 : t_cost_data_size
        
        t_cost_data_elu(i, 1) = i;
        t_cost_data_elu(i, 2) = t_record_cost_data(i);
        
    end
    
    plot(t_cost_data_elu(:,1), t_cost_data_elu(:,2),'--');
    
    s = input('save the loss data?, y to save:','s');
    if(s == 'y')
        save('data_loss_elu_softmax.mat', 't_cost_data_elu');
    end
end
...
while(t_test)
    ...
    t_layer_one_data = function_Utils_ELuFunction(t_picked_Image_data * t_layer_one_weight', t_elu_param);
...
t_layer_one_data = function_Utils_ELuFunction(t_input_data_for_evaluate * t_layer_one_weight', t_elu_param);
