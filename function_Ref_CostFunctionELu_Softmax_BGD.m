...
%the neuron activation is using ELu
function [r_cost, r_gradient] = function_Ref_CostFunctionELu_Softmax_BGD(p_input_weight, p_input_data, p_answer_data, p_layer_one_weight_size, p_layer_two_weight_size, p_regularization_param, p_elu_param)
    %unpack the weight
    ...
    %the layer two input
    %add additonal 1 column at the begining
    t_layer_two_input_data = function_Utils_ELuFunction(t_layer_one_output_data, p_elu_param);
   ...
   t_layer_two_error = t_layer_three_error * t_layer_two_weight;
    ...
    t_layer_one_error = t_layer_two_error .* function_Utils_ELuGradientFunction(t_layer_one_output_data, p_elu_param);
